...
```
npm install --save-dev react-spa-static-prerender
```
...
```json
"scripts": {
  "prerender": "react-spa-static-prerender --with-build"
}
```
...
| `generateSitemap` | `boolean` | `false` | Generate a sitemap.xml in `outDir` based on `routes` |
...
| `origin` | `string` | `""` | Optional origin to prepend to URLs when generating the sitemap |
...
* [Janko Stanic](https://github.com/jankojjs) for creating the [original version](https://github.com/jankojjs/react-static-prerender)
